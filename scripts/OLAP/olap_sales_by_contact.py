...
def create_olap_cube(
    ...
    try:
        ...
        cube["TransactionIDs"] = grouped["transaction_id"].apply(list).reset_index(drop=True)
...
def main():
    ...
    print(sales_df.columns)  # Inspect the column names

    # Step 2: Add additional columns for time-based dimensions
    sales_df["sale_date"] = pd.to_datetime(sales_df["sale_date"])
    sales_df["DayOfWeek"] = sales_df["sale_date"].dt.day_name()
    sales_df["Month"] = sales_df["sale_date"].dt.month  # Add Month column
    sales_df["Year"] = sales_df["sale_date"].dt.year

    # Step 3: Define dimensions and metrics for the cube
    # Change 'ProductID' to 'product_id' to match the column name in the DataFrame
    dimensions = ["DayOfWeek", "Month", "product_id", "customer_id"]  # Updated to 'product_id'
    metrics = {
        "sale_amount": ["sum", "mean"],  # Ensure 'sale_amount' matches the column in the DataFrame
        "transaction_id": "count"  # Use 'transaction_id' as per the available column
    }
