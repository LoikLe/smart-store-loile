...
def analyze_products_sold_by_month(cube_df: pd.DataFrame, products_df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        merged_data = grouped.merge(
            products_df[["ProductID", "ProductName"]],  # Adjusted column names here
            left_on="product_id",  # Merge on 'product_id'
            right_on="ProductID",  # Merge on 'ProductID' in products_df
            how="left"
        )
